...
async def intercept_handler(client: Client, msg: Message) -> None:
    if get_var("FORCEJOIN") != "":
        try:
            ...
            await msg.reply_text("Join the given chat in order to use this bot.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Updates Channel", url=f"{forcejoin}")]]), parse_mode="HTML")
            return
